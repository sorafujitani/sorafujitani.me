...
なお、次の章 ([TypeScript入門](/blog/ts-first-step/)) では `js-playground` とは別に `ts-playground` という新しいディレクトリを作って、そこをそのまま第3章 (Hono) と第4章 (型と tsconfig) でも使い回す。
JS用と TS用でディレクトリを分けるだけで、それ以上のフォルダ構造は無し。
