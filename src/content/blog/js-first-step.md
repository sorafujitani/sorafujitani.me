...
各セクションのコードは `01-js/index.js` の中身に貼り付けて、そのつど実行しながら読むのがおすすめ。読むだけより、出力を見ながら進めたほうが頭に入る。
...
このシリーズは全4章を通して、`tsbook` という1つのディレクトリで作業する。
章ごとに `01-js/`、`02-ts/`、`03-hono/`、`04-types/` というサブディレクトリを掘って、書いたコードはそこに残していく。章を進めても前のコードが消えないので、後から見返したり比べたりしやすい。

最初の1回だけ、ルートのセットアップをする。

```bash
mkdir tsbook && cd tsbook
bun init -y                  # package.json と tsconfig.json をデフォルト値で生成
mkdir 01-js
touch 01-js/index.js
```

`bun init -y` を一度走らせれば、以降の章で必要になる `tsconfig.json` もここでまとめて用意される。
まずは環境チェック。`01-js/index.js` を以下の内容にする。
...
これが出れば準備OK。以降のコードはすべて `01-js/index.js` の中身をまるごと書き換えて、同じコマンドで実行できる。**1セクションごとに動かしながら**進めるのが一番理解が早い。
