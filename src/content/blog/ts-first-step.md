...
[前回](/blog/js-first-step/) と同じ `tsbook` ディレクトリで作業を続ける。今回はTypeScriptを書くので、章ごとのサブディレクトリを増やすだけでよい。

```bash
# tsbook/ の中で
mkdir 02-ts
touch 02-ts/index.ts
```

第1章で `bun init -y` した時点で `tsbook/tsconfig.json` が生成されているので、追加設定は要らない。`02-ts/index.ts` にコードを書いて `bun run 02-ts/index.ts` で実行できる。
